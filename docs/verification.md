...
- [Workflow run #14](https://github.com/TopPlayer254/yandex-music-api-ios/actions/runs/34397295689)
...
- `xcodebuild archive` produced an unsigned arm64 device archive after compiling the artwork-backed
  Wave card, its bounded SwiftUI layer-effect shader, local MP3 playlist import, dynamic mini-player
  accessory and the immediate playback-selection path. The IPA contains `default.metallib`.
- The archive was packaged as `dist/MapleMusic-unsigned.ipa`; local SHA-256:
  `f944ca6b731fb79de41bfcaf5d1bbabb039a438ce030a7879cf5ccc87e2c60ef`.
- The IPA structure contains `Payload/MapleMusic.app/Info.plist` and no
  `embedded.mobileprovision` or `_CodeSignature` entry.
- The packaged app reports bundle ID `com.hikeri.yamusic`, version `0.10.0`, build `14`,
...
backward decoding of existing offline lyrics metadata, and persisted local MP3 playlist membership.
